...
export function StoryViewer({ stories, videoUrl, videoDate }: {
  ...
  const prev = () => setCurrentIdx(p => (p - 1 + stories.length) % stories.length);
  const next = () => setCurrentIdx(p => (p + 1) % stories.length);
  ...
  return (
    <div>
      ...
      <div className="px-6 md:px-12 py-3 flex items-center gap-3" style={{ background: '#f5f5f5' }}>
        <span className="text-[10px] font-bold text-[#1e2a3a] bg-[#1e2a3a]/10 px-2 py-0.5 rounded uppercase tracking-[0.1em]">
          Story {currentIdx + 1} of {stories.length}
        </span>
      ...
      {/* FULL STORY CONTENT — always expanded */}
