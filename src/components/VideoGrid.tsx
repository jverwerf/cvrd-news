...
export function VideoGrid({ youtubeVideos, socialClips, storyImage }: {
  ...
  // X, TikTok, Reels → social cards below the player
  ...
  useEffect(() => {
    if (!playing || !active) { stopPolling(); stopTimer(); return; }
    startPolling();
  ...
  return (
    <div className="mb-6">
        ...
        <div className="rounded-md overflow-hidden border border-[#e5e5e5]">
          ...
          <div className="flex items-center px-3 py-2 bg-[#fafafa] border-t border-[#f0f0f0]">
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              <span className="w-[5px] h-[5px] rounded-full shrink-0 bg-[#ff0000]" />
      ...
      {items.length > 1 && (
        <div className="mt-3">
          <div className="h-2 rounded-full overflow-hidden bg-[#e5e5e5] mb-2 cursor-pointer"
            ...
            <div className="h-full rounded-full transition-all duration-500 ease-linear bg-[#ff0000]" style={{
              width: `${progress * 100}%`,
            }} />
          </div>
            ...
            <div id={`thumbs-${items[0]?.embed_id}`} className="flex gap-1 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
              {items.map((item, i) => (
                <button key={i} onClick={() => { setActiveIdx(i); setCurrentTime(0); setDuration(0); setPlaying(true); }}
                  ...
                  style={{
                    width: '100px',
                    border: i === activeIdx ? '2px solid #ff0000' : '2px solid transparent',
      ...
      {/* SOCIAL CARDS — X, TikTok, Reels shown below the player */}
      {cards.length > 0 && (
        <div className="mt-3 flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {cards.map((card, i) => (
            <div key={i} className="shrink-0 rounded-md overflow-hidden border border-[#e5e5e5]" style={{ width: card.platform === 'x' ? '320px' : '200px' }}>
              {card.platform === 'x' && (
                <div className="light"><Tweet id={card.embed_id} /></div>
              )}
              {card.platform === 'tiktok' && (
                <iframe src={`https://www.tiktok.com/embed/v2/${card.embed_id}`}
                  className="w-full h-[400px]" allowFullScreen allow="encrypted-media" />
              )}
              {card.platform === 'reels' && (
                <iframe src={`https://www.instagram.com/reel/${card.embed_id}/embed`}
                  className="w-full h-[400px]" allowFullScreen />
              )}
            </div>
          ))}
        </div>
