...
const C = {
  ...
  gold: '#daa520', text: '#e2e8f0', dim: '#7a8fa6', dimmer: '#4a5a6a',
...
export default function BreakingDetailClient({ story, raw, type }: {
  ...
  const totalClips = ytVids.length + clips.filter(c => c.duration).length;
    ...
    <>
      ...
      <div style={{ background: C.bg, color: C.text, minHeight: '100vh', fontFamily: sans }}>
        ...
        <div style={{ position: 'relative', overflow: 'hidden' }}>
          {firstThumb ? (
            <div style={{
              ...
              backgroundImage: `url(${firstThumb})`,
              backgroundSize: 'cover', backgroundPosition: 'center',
              filter: 'brightness(0.35) blur(2px)',
            }} />
          ) : (
            <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${type === 'breaking' ? '#7f1d1d' : '#1d4ed8'}22, ${C.panelDark})` }} />
          )}
          ...
          <div style={{ position: 'relative', padding: '52px 28px 20px', maxWidth: 1120, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }} className="hero-meta-row">
              <a href="/breaking" style={{ fontFamily: mono, fontSize: 10, letterSpacing: '0.1em', color: C.dim, textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{
                ...
                {type === 'breaking' && <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#fff' }} className="live-dot" />}
        ...
        <main style={{ maxWidth: 1120, margin: '0 auto', padding: '24px 16px 40px' }}>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-0 rounded-lg" style={{ background: C.panel }}>
                <div className="py-4 px-4 md:border-r md:border-b-0 border-b" style={{ borderColor: '#2a3a4a' }}>
                  ...
                  <p className="text-[13px] text-[#bbb] leading-[1.65]">{story.left_narrative || 'No coverage at time of publication.'}</p>
                </div>
                <div className="py-4 px-4 md:border-r md:border-b-0 border-b" style={{ borderColor: '#2a3a4a' }}>
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-[13px] text-[#bbb] leading-[1.65]">{story.center_narrative || 'No coverage at time of publication.'}</p>
                </div>
                <div className="py-4 px-4">
                  ...
                  <p className="text-[13px] text-[#bbb] leading-[1.65]">{story.right_narrative || 'No coverage at time of publication.'}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {/* X POSTS */}
          {xClips.length > 0 && (
            <div className="mb-8" data-section="x-posts">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[16px] font-bold text-white">{'\u{1D54F}'}</span>
              </div>
              <div className="rounded-lg p-4" style={{ background: C.panel }}>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                  {xClips.slice(0, 8).map((c, i) => (
                    <div key={i} className="rounded overflow-hidden relative" style={{ background: C.bg, height: 90 }}>
              ...
              <div className="rounded-lg p-4" style={{ background: C.panel }}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {tiktokClips.slice(0, 6).map((c, i) => (
                    <div key={i} className="rounded-md overflow-hidden" style={{ background: C.bg }}>
          ...
          {telegramClips.length > 0 && (
            <div className="mb-8" data-section="telegram">
              <div className="flex items-center gap-2 mb-3">
